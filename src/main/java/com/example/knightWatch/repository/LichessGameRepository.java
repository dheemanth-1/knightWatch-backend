...
public interface LichessGameRepository extends JpaRepository<LichessGame, Long> {
    ...
    String findLatestGameDateByUsername(@Param("username") String username);@Query(value = "SELECT * FROM lichess_games WHERE username = :username " +
            "ORDER BY strftime('%s', " +
            "  substr(played_at, 1, 4) || '-' || " +     // Year: "2022"
            "  substr(played_at, 6, 2) || '-' || " +     // Month: "09"
            "  substr(played_at, 9, 2) || 'T' || " +     // Day: "09"
            "  substr(played_at, 11, 8)" +               // Time: "14:20:21"
            ") DESC LIMIT 1",
            nativeQuery = true)
